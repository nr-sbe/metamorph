...
import './style.css';
import {MobileControls,prefersTouch} from './mobile';
...
const settings={gore:true,shake:true,sound:true,quality:prefersTouch?'performance':'high',music:.32,effects:.7};

try{Object.assign(settings,JSON.parse(localStorage.getItem('morph-settings')||'{}'));}catch{}

const mobile=new MobileControls(canvas,{
 active:()=>ready&&game.running&&!game.paused&&!game.wheel&&el('menu').classList.contains('hidden'),
 press:code=>{gamepadActive=false;game.press(code);},
 release:(code,cancelled)=>{if(!game)return;if(cancelled&&code==='Mouse0')game.cancelCharge();if(cancelled&&code==='Space'){game.keys.delete(code);game.just.delete(code);game.jumpCharge=0;return;}game.release(code);},
 look:(x,y)=>{gamepadActive=false;game.cameraYaw+=x;game.cameraPitch=clamp(game.cameraPitch+y,-.7,.9);},
 resize:()=>engine?.resize(),wake:()=>{if(ready){fx.sound.start();fx.sound.pauseMusic(game.paused);}}
});
...
function lock(){if(mobile.enabled)return;canvas.focus();try{const result=canvas.requestPointerLock?.();if(result&&'catch' in result)result.catch(()=>game.say('Mouse capture unavailable. Hold left Alt and drag to look.'));}catch{game.say('Hold left Alt and drag to look.');}}

function begin(mode:string){if(!ready)return;mobile.clear();fx.sound.start();game.start(mode);el('menu').classList.add('hidden');el('panel').classList.add('hidden');el('hud').classList.remove('hidden');el('resume-menu').classList.remove('hidden');lock();}

function pause(){if(!game?.running)return;mobile.clear();game.paused=true;padSprint=false;fx.sound.pauseMusic(true);game.release('Block');game.keys.clear();game.moveStick=null;game.wheel=false;game.charging=false;game.jumpCharge=0;document.exitPointerLock?.();el('wheel').classList.add('hidden');showPause();}
...
canvas.addEventListener('contextmenu',e=>e.preventDefault());canvas.addEventListener('pointerdown',e=>{if(e.pointerType==='touch')return;e.preventDefault();if(!ready||!game.running||game.paused)return;if(e.altKey){dragging=true;lastPointer={x:e.clientX,y:e.clientY};return;}if(document.pointerLockElement!==canvas)lock();gamepadActive=false;const command=mouseCommand(e.button);if(command)game.press(command);});document.addEventListener('pointerup',e=>{if(e.pointerType==='touch')return;dragging=false;if(ready){const command=mouseCommand(e.button);if(command)game.release(command);}});document.addEventListener('pointermove',e=>{if(e.pointerType==='touch')return;if(!ready||game.paused||game.wheel)return;if(document.pointerLockElement===canvas){game.cameraYaw+=e.movementX*.0025;game.cameraPitch=clamp(game.cameraPitch+e.movementY*.002,-.7,.9);}else if(dragging){game.cameraYaw+=(e.clientX-lastPointer.x)*.004;game.cameraPitch=clamp(game.cameraPitch+(e.clientY-lastPointer.y)*.003,-.7,.9);lastPointer={x:e.clientX,y:e.clientY};}});
...
async function boot(){try{
 ...
 game.onEvent=name=>{if(name==='reset'){mobile.clear();accumulator=0;renderPose.reset(game.pos,game.facing);followCamera.reset();cameraFraction=1;}if(name==='death')pause();if(name==='damage'){el('damage').style.opacity='.55';setTimeout(()=>el('damage').style.opacity='0',180);}};
 ...
 mobile.update();if(game.running)pad(dt);if(mobile.enabled&&mobile.movement&&!game.paused&&!game.wheel)game.moveStick=mobile.movement;if(game.running&&!game.paused){let sim=dt*(game.wheel?.12:1);if(fx.hitPause>0){fx.hitPause-=dt;sim*=.13;}accumulator=Math.min(accumulator+sim,.12);while(accumulator>=1/60){renderPose.beforeStep(game.pos);game.step(1/60);accumulator-=1/60;}}
